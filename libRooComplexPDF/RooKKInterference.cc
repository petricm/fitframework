...
ClassImp(RooKKInterference)

RooKKInterference::RooKKInterference(const char *name, const char *title,
		RooAbsReal& _m,
		RooAbsReal& _phi,
		RooAbsReal& _cos_th1,
		RooAbsReal& _cos_th2,
...
RooKKInterference::RooKKInterference(const RooKKInterference& other, const char* name) :
...
Double_t RooKKInterference::evaluate() const {
	...
	KK_flatte = std::polar(std::sin(std::arg(KK_flatte)),std::arg(KK_flatte));
...
Int_t RooKKInterference::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /* rangeName */) const {
...
Double_t RooKKInterference::analyticalIntegral(Int_t code, const char* rangeName ) const {
	...
	if (code == 5) {
		...
		// interference terms cancel due to acceptance, so we don't add them
