#pragma once
/**
 * @file	RooDalitzAnalysisCoreCachedIntegral.h
 * @date	Nov 11, 2013
 * @author	mprim
...
#include "libRooComplexPDF/RooRooDalitzAnalysis.h"

class RooDalitzAnalysisCoreCachedIntegral : public CachedIntegral {
public:
...
protected:
	RooDalitzAnalysisCoreCachedIntegral() : CachedIntegral() {};
	RooDalitzAnalysisCoreCachedIntegral(const std::vector<std::pair<double, double> >& limits,
	...
	std::complex<double> GetS(const double m, const double phi, const double th1, const double th2,
		...
		if(J==0) {
			...
			std::cerr << "ERROR: Spin " << J << " not implemented for GetS(..) in RooDalitzAnalysisCore" << std::endl;
