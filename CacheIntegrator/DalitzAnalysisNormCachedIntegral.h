#pragma once
/**
 * @file	DalitzAnalysisNormCachedIntegral.h
 * @date	Nov 11, 2013
 * @author	petric
...
#ifndef DALITZANALYSISNORMCACHEDINTEGRAL_H_
#define DALITZANALYSISNORMCACHEDINTEGRAL_H_
 ...
 */
class DalitzAnalysisNormCachedIntegral : public DalitzAnalysisCoreCachedIntegral {
public:
	DalitzAnalysisNormCachedIntegral() : DalitzAnalysisCoreCachedIntegral() {};
	DalitzAnalysisNormCachedIntegral(const std::vector<std::pair<double, double> >& limits,
	...
	virtual double DoEval(const double* x) const {
		...
		return GetEff(mass_AB,mass_BC,eff_func) * std::norm(S);
	}

	virtual ROOT::Math::IMultiGenFunction* Clone() const {
		DalitzAnalysisNormCachedIntegral* f = new DalitzAnalysisNormCachedIntegral(*this);
		return f;
	}

	virtual std::string GetIntegralClassIdentifier() const {
		return "DalitzAnalysisNormCachedIntegral";
	}
};

#endif /* DALITZANALYSISNORMCACHEDINTEGRAL_H_ */
