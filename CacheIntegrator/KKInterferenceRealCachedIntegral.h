#pragma once
/**
 * @file	KKInterferenceRealCachedIntegral.h
 * @date	May 23, 2011
 * @author	mprim
...
#ifndef KKINTERFERENCEIREALCACHEDINTEGRAL_H_
#define KKINTERFERENCEIREALCACHEDINTEGRAL_H_
 ...
 */
class KKInterferenceRealCachedIntegral : public CachedIntegral {
public:
	KKInterferenceRealCachedIntegral() : CachedIntegral() {};
	KKInterferenceRealCachedIntegral(const std::vector<std::pair<double, double> >& limits,
	...
	virtual double DoEval(const double* x) const {
		...
		return GetEff(m3,phi,th1,th2,eff_func) * (KK_flatte*angular_flatte * std::conj(KK_phi * angular_phi)).real() * F;
	}

	virtual ROOT::Math::IBaseFunctionMultiDim* Clone() const {
		KKInterferenceRealCachedIntegral* f = new KKInterferenceRealCachedIntegral(*this);
		return f;
	}

	virtual std::string GetIntegralClassIdentifier() const {
		return "KKInterferenceRealCachedIntegral";
	}
};
