#pragma once
/**
 * @file	KKInterferenceImagCachedIntegral.h
 * @date	May 23, 2011
 * @author	mprim
...
#ifndef KKINTERFERENCEIMAGCACHEDINTEGRAL_H_
#define KKINTERFERENCEIMAGCACHEDINTEGRAL_H_
 ...
 */
class KKInterferenceImagCachedIntegral : public CachedIntegral {
public:
	KKInterferenceImagCachedIntegral() : CachedIntegral() {};
	KKInterferenceImagCachedIntegral(const std::vector<std::pair<double, double> >& limits,
	...
	virtual double DoEval(const double* x) const {
		...
		return GetEff(m3,phi,th1,th2,eff_func) * (KK_flatte*angular_flatte * std::conj(KK_phi * angular_phi)).imag() * F;
	}

	virtual ROOT::Math::IBaseFunctionMultiDim* Clone() const {
		KKInterferenceImagCachedIntegral* f = new KKInterferenceImagCachedIntegral(*this);
		return f;
	}

	virtual std::string GetIntegralClassIdentifier() const {
		return "KKInterferenceImagCachedIntegral";
	}
};
