#pragma once
/**
 * @file	DalitzAnalysisRealCachedIntegral.h
 * @date	Nov 11, 2013
 * @author	petric
...
#ifndef DALITZANALYSISREALCACHEDINTEGRAL_H_
#define DALITZANALYSISREALCACHEDINTEGRAL_H_
 ...
 */
class DalitzAnalysisRealCachedIntegral : public DalitzAnalysisCoreCachedIntegral {
public:
	DalitzAnalysisRealCachedIntegral() : DalitzAnalysisCoreCachedIntegral() {};
	DalitzAnalysisRealCachedIntegral(const std::vector<std::pair<double, double> >& limits,
	...
	virtual double DoEval(const double* x) const {
		...
		return GetEff(mass_AB,mass_BC,eff_func) * (S * S_cc).real();
	}

	virtual ROOT::Math::IMultiGenFunction* Clone() const {
		DalitzAnalysisRealCachedIntegral* f = new DalitzAnalysisRealCachedIntegral(*this);
		return f;
	}

	virtual std::string GetIntegralClassIdentifier() const {
		return "DalitzAnalysisRealCachedIntegral";
	}
};

#endif /* DALITZANALYSISREALCACHEDINTEGRAL_H_ */
